...
public class NPCStatsCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§3/p4 npcstatus adicionar [id] §f- §7Adicionar NPC.\n§3/p4 npcstatus remover [id] §f- §7Remover NPC.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /p4 npcstatus adicionar [id]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /p4 npcstatus remover [id]");
        return;
      }
